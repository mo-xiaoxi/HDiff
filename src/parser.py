...
import pathlib
import typing
from src.util import flatten, format_abnf
...
class Rule:
    ...
    def __new__(cls, name, children=None):  # pylint: disable=unused-argument
    ...
    def __init__(self, name, children=None):
        ...
        if children is not None:
            # when defined-as = '=/', we'll need to overwrite existing children.
    ...
    def parse(self, source, start=0):
        ...
        :raises GrammarError: if rule has no children.  This usually means that a
            non-terminal in the grammar is not defined or imported.
        """
        try:
            try:
                node, new_start = self.children.parse(source, start)
    ...
    @classmethod
    def from_txt(cls, rule_source):
        """多行读入规则"""
        rule_source = format_abnf(rule_source)
        node = ABNFGrammarRule("rulelist").parse_all(rule_source)
...
for grammar_rule_def in [
    ...
    (
            ...
            Alternation(
                ...
                Concatenation(Option(Literal("%s")), ABNFGrammarRule("single-quoted-string")),
            ),
    ),
    (  # added by moxiaoxi
            ...
            Concatenation(
                ...
                Repetition(
                    Repeat(),
                    Alternation(Literal(("\x20", "\x26")), Literal(("\x28", "\x7E"))),
                ),
                Literal("\x27"),
            ),
    ),
...
class CharValNodeVisitor(NodeVisitor):
    ...
    @staticmethod
    def visit_single_quoted_string(node):
        """Visit a quoted-string node."""
        return node.value[1:-1]
...
class ABNFGrammarNodeVisitor(NodeVisitor):
    ...
    def visit_rule(self, node):
        ...
        rule.children = (
            elements if defined_as == "=" else Alternation(rule.children, elements)
        )
        return rule
